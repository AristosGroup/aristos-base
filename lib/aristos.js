...
Aristos.prototype.setupMainMenu = function() {
    AristosUtils.debug('Вызов метода Aristos.setupMainMenu');
    var self = this;
    Deps.autorun(function(){
        var modules = AristosModules.find();
        AristosUtils.debug('Количество модулей для конфигурации: ', modules.count());
        if(modules.count() > 0) {
...
Aristos.prototype.setupModules = function () {
    AristosUtils.debug('Вызов метода Aristos.setupModules');
    var self = this;
...
Aristos.prototype.moduleInstanceByKey = function(key, moduleConfig) {
    ...
    if (Aristos[moduleClassName] != undefined) {
        ...
        console.warn('Внимание: модуль ' + moduleClassName + ' не существует.');
