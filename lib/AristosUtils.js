...
AristosUtils.JSON = {
        ...
        _.each(obj, function(elem, key) {
            if(elem.hasOwnProperty('type')) {
                switch (elem.type) {
                    ...
                    case 'Number': objectified[key] = parseFloat(elem.value); break;
    ...
    /**
     * Обычный парсер JSON + распознание регулярных выражений из строки
     * @param json
     * @returns {*}
     */
    simpleParserWithRegEx: function(json) {
        var self = this,
        ...
        _.each(obj, function(elem, key) {
            switch (typeof elem) {
                case 'string':
                    ...
                    if(res) {
                        res = res.slice(1);
                        obj[key] = new RegExp(res[0], res[1]);
                    }
                    break;
                case 'object':
                    obj[key] = self.simpleParserWithRegEx(elem);
                    break;
            }
        });
        return obj;
