...
AristosModuleBase = function (key, moduleConfig) {
    ...
    self.safeConfig = ['name', 'config', 'roles', 'subItems', 'menu'];
    self.key = key;
...
AristosModuleBase.prototype._updateCollection = function () {
    ...
    console.log('Обновление конфигурации модуля '+this.name+'\n' +
        'Текущий конфиг (self.savedConfig):', self.savedConfig, '\n',
        'self.moduleConfig:', self.moduleConfig,'\n' +
        'self.safeConfig:', self.safeConfig, '\n'
    );

    var moduleConfig = _.pick(self.moduleConfig, self.safeConfig);

    console.log(
        'moduleConfig = _.pick(self.moduleConfig, self.safeConfig):', moduleConfig, '\n'
    );
    ...
    if(moduleConfig.subItems) {
        _.each(moduleConfig.subItems, function(subItem, key){
            //Проверяем наличие модуля в существующем конфиге
            if(_.where(module.subItems, { key: subItem.key }).length == 0) {
                if(typeof newSettings.subItems == "undefined")
                    newSettings.subItems = module.subItems;
                newSettings.subItems.push(subItem);
            }
        });
    }

    if (_.size(newSettings) > 0) {
        console.log('Обновляем конфигурацию: ', newSettings);
