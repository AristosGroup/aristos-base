        ...
        _.each(module.subItems, function (subItemData) {
            ...
            if(subItemData.router) {
                ...
                callbacks.push(function () {
                    ...
                    checkModule.checkAccess(subModuleKey);
                });
...
AristosModuleBase.prototype.checkAccess = function(subModuleKey, user) {
    if(!this.subItemsIndex.hasOwnProperty(subModuleKey)) throw 'Несуществующий сабмодуль';
    if(!user) user = Meteor.user();
    if (!user || !Roles.userIsInRole(user, this.subItems[this.subItemsIndex[subModuleKey]].roles)) {
        console.log(user, this.subItems[this.subItemsIndex[subModuleKey]].roles);
        Log('Access Denied by module config');
        Router.go('/403');
        return false;
    }
    return true;
